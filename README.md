...
# compare against a high-precision box-filter oracle, Skia, and Slug (synthetic shapes + glyphs a–z)
# (--convergence for the reference-refinement study, --selftest to cross-check the oracle itself)
deno task validate
...
- Slug approximates the box filter; windfoil computes it (nearly) exactly in all cases I tested and care about — `deno task validate` measures windfoil against a high-precision box-filter oracle at ~10⁻⁴ mean over affected edge pixels (dominated by GPU vertex snapping, not the integral — see [docs/ALGORITHM.md §5](docs/ALGORITHM.md)), vs ~10⁻² for Slug and Skia. To be clear about what that metric means: Skia's anti-aliasing deliberately implements its own reconstruction model rather than the box filter, so its distance from this target reflects a different target, not lower quality — and on the pathological pixels containing three-plus overlapping winding levels, Skia is the more correct renderer; as far as I understand no single-sample approach can handle those (windfoil and Slug tie there). Separately, tested across a ~1000 shape corpus, windfoil matched Skia's own output more closely than Slug did: Δ 0.164/255 vs 0.189, and every outlier improved.
